...
#define SIZE        10000  //(L*L)    /* number of sites                */
...
void createLink(void)
{
	...
	femi = abs(player_payoff[player1] - player_payoff[player2]) / (max(player_degree[player1], player_degree[player2])*b);
	//第三种办法，黄老师的归一化

	//for (int num = 0; num<SIZE; num++)
	//{
	//	player_delta[num] = abs(player_payoff[num] - averageGain);
	//}
	//for (int num1 = 0; num1<SIZE; num1++)
	//{
	//	for (int num2 = 0; num2<SIZE; num2++)
	//	{
	//		sumDelta += max(player_delta[num1], player_delta[num2]) / min(player_delta[num1], player_delta[num2]);
	//	}
	//}
	//sumDelta = (sumDelta / SIZE) / 2;
	//femi = max(player_delta[player1], player_delta[player2]) / min(player_delta[player1], player_delta[player2]) / sumDelta;
	//第二种方法

	//femi = abs(player_payoff[player1]-player_payoff[player2]) / (2*averageGain);
	//第一种方法

	if (femi > 1)	cout << "femi is greater than 1" << '\t' << femi << endl;
